...
###############################
# parameters (these NEED to be loaded manually)
###############################
...
###############################
# 1. load metadata for the CUBIC GRATING
###############################

with open("RESULTS_3D_metadata.txt", "r") as f:
    lines = f.readlines()
    num_phi1, phi1_min, phi1_max = map(float, lines[0].split()) # assign three variables with a split single line
    num_gamma2, gamma2_min, gamma2_max = map(float, lines[1].split())
    num_tau, tau_min, tau_max = map(float, lines[2].split())

num_phi1, num_gamma2, num_tau = int(num_phi1), int(num_gamma2), int(num_tau) # cast, just to be completely sure...

###############################
# create coordinate vectors (initialize checkerboard...)
###############################
...
###############################
# 4. interactive slider for \gamma_2
###############################

slider_ax = plt.axes([0.20, 0.05, 0.65, 0.03])
gamma2_slider = Slider(
    ax=slider_ax,
    label=rf'$\gamma_2$ Index',
    valmin=0,
    valmax=num_gamma2 - 1,
    valinit=initial_g_idx,
    valfmt='%d' # how it's formatted
)
...
def update(val):
    ...
    ax.set_title(rf"$\gamma_2 = {gamma2_vals[g_idx]:.3f}$")
